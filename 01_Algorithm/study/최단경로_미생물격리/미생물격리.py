...
def separate():
    ...
    while group:  # 일단 전부 하나씩 꺼내서 이동 시킴
        x, y, cnt, dir = group.popleft()
        x += dx[dir]
        y += dy[dir]  # 일단 이동 시킨 후
    ...
    for (x, y), value in temp.items():
        if len(value) == 1:  # 해당 위치에 하나의 군집만
            group.append((x, y, value[0][0], value[0][1]))
        else:  # 여러 개
            max_cnt = 0
            max_dir = -1
            sum_cnt = 0
            for cnt, dir in value:  # 하나씩 꺼내기
                sum_cnt += cnt
                if cnt > max_cnt:
                    max_cnt = cnt
                    max_dir = dir
            group.append((x,y,sum_cnt,max_dir))
